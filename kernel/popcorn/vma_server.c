		...
		if (operation->operation == VMA_OP_MAP
			...
			if (operation->from_nid != my_nid) {
				printk("%s: ERROR: the server pushed me an operation %i of cpu %i\n",
				       __func__, operation->operation, operation->from_nid);
				up_write(&mm->mmap_sem);
				pcn_kmsg_free_msg(operation);
				kfree(work);
				return ;
	...
	if (start_ret == VMA_OP_SAVE) {
		...
		*/
		...
		if (entry->message_push_operation != NULL) {
			...
			case VMA_OP_MAP:
			case VMA_OP_BRK:
				err = pcn_kmsg_send_long(entry->message_push_operation->from_nid,
				...
				if (err == -1)
					printk("%s: ERROR: impossible to send operation %d to client in cpu %d\n",
							__func__, operation, entry->message_push_operation->from_nid);
				else {
