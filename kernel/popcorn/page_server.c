...
/* TODO */
/* TODO1: use define & undefine */
/* TODO2: no top bottom */
#define PREFETCH_BACKGROUND 1
#define PREFETCH_REMOTE_REMOTE_SUPPORT 1
#define PREFETCH_REMORE_NONBLOCKING 0
/* 0: didn't test it for a while */
#define PREFETCH_REMOTE_REMOTE_IMM 1

unsigned long g_pf_req_id = 0; /* Prefetch request id generator */
...
/* Ongoing prefetching request mapping table per batched request */
...
/* Temporary map table for saving non-blocking remote page fetch's metadata */
struct remote_remote_req_map {
	bool is_rr[MAX_PF_REQ];

	spinlock_t *ptl[MAX_PF_REQ];
	int succ_pf_num[MAX_PF_REQ];
	unsigned long addr[MAX_PF_REQ];
	unsigned long fault_flags[MAX_PF_REQ];
	struct fault_handle *fh[MAX_PF_REQ];

	pte_t *pte[MAX_PF_REQ];
	unsigned long *pi[MAX_PF_REQ];
	struct page *pip[MAX_PF_REQ];
	struct wait_station *ws[MAX_PF_REQ];
	struct remote_context *rc[MAX_PF_REQ];
	struct pcn_kmsg_rdma_handle *rh[MAX_PF_REQ];
};

/* Temporary map table for preprefetch */
...
atomic_t a0, a1, a2, a3;	/* prefetch at remote req */
atomic_t b0, b1, b2;		/* prefetch at remote res */
atomic_t c0, c1, c3;		/* prefetch at origin */
atomic64_t c2;
...
enum pf_action_code {
	...
	/* REQ */
	PF_READ_REQ = 3,	/* Can be calculated */
	PF_WRITE_X_REQ,		/* Can be calculated */
	PF_WRITE_O_REQ,		/* Can be calculated */
	...
	/* FAIL */
	PF_READ_RES_FAIL = 9,
	PF_WRITE_X_RES_FAIL,	/* Not differenciate WRITE_FAIL currently */
	PF_WRITE_O_RES_FAIL,
...
#ifdef CONFIG_POPCORN_STAT
	if (seq) {
		seq_printf(seq, "%2s  %12ld,%06ld   %3s %-12ld   %3s %-12ld   %2s  %-6s   %3s  %12d\n",
						"mm", (atomic64_read(&mm_time_ns) / 1000) / MICROSECOND,
								(atomic64_read(&mm_time_ns) / 1000)  % MICROSECOND,
						"cnt", atomic64_read(&mm_cnt),
						"per", atomic64_read(&mm_cnt) ? atomic64_read(&mm_time_ns) / atomic64_read(&mm_cnt) / 1000: 0,
						"us", "(unit)",
						"cut", atomic_read(&madv_filtered_pg_cnt));
		seq_printf(seq, "%2s  %12ld,%06ld   %3s %-12ld   %3s %-12ld   %2s  %-12ld   %2s  %-12ld\n",
						"pf", atomic64_read(&pf_time_u) / MICROSECOND,
								atomic64_read(&pf_time_u) % MICROSECOND,
        ...
        seq_printf(seq, "-----------------------------------------------------------\n");
        seq_printf(seq, "%3s %12d  %3s %-12d  %3s %-12d  %2s %-12d  %2s %-d\n",
                        "a0", atomic_read(&a0),
                        "a1", atomic_read(&a1),
                        "a2", atomic_read(&a2),
                        "a3", atomic_read(&a3),
                        "ns", 0);
		seq_printf(seq, "%3s %12d  %3s %-12d  %3s %-12d  %2s %-12d  %2s %-12d\n",
						"avg", 0,
						"a1", atomic_read(&a0) ? atomic_read(&a1) / atomic_read(&a0) : 0,
						"a2", atomic_read(&a0) ? atomic_read(&a2) / atomic_read(&a0) : 0,
						"a3", atomic_read(&a0) ? atomic_read(&a3) / atomic_read(&a0) : 0,
						"ns", 0);
		seq_printf(seq, "%3s %12d  %3s %-12d  %3s %-12d  %2s %-12d  %2s %-12d\n",
						"avg", 0,
						"a1", atomic_read(&a0) ? (atomic_read(&a1) / atomic_read(&a0))/1000 : 0,
						"a2", atomic_read(&a0) ? (atomic_read(&a2) / atomic_read(&a0))/1000 : 0,
						"a3", atomic_read(&a0) ? (atomic_read(&a3) / atomic_read(&a0))/1000 : 0,
						"us", 0);
		...
		seq_printf(seq, "%3s %12d  %3s %-12d  %3s %-12d  %2s %-12d  %2s %-12d\n",
						"avg", 0,
						"b1", atomic_read(&b0) ? atomic_read(&b1) / atomic_read(&b0) : 0,
						"b2", atomic_read(&b0) ? atomic_read(&b2) / atomic_read(&b0) : 0,
						"xx", 0,
						"ns", 0);
		seq_printf(seq, "%3s %12d  %3s %-12d  %3s %-12d  %2s %-12d  %2s %-12d\n",
						"avg", 0,
						"b1", atomic_read(&b0) ? (atomic_read(&b1) / atomic_read(&b0))/1000 : 0,
						"b2", atomic_read(&b0) ? (atomic_read(&b2) / atomic_read(&b0))/1000 : 0,
						"xx", 0,
						"us", 0);

		seq_printf(seq, "%3s %12d  %3s %-12d  %3s %-12ld  %2s %-12d  %2s %-d\n",
                        "c0", atomic_read(&c0),
                        "c1", atomic_read(&c1),
                        "c2", atomic64_read(&c2),
                        "c3", atomic_read(&c3),
                        "ns", 0);
		seq_printf(seq, "%3s %12d  %3s %-12d  %3s %-12ld  %2s %-12d  %2s %-12d\n",
						"avg", 0,
						"c1", atomic_read(&c0) ? atomic_read(&c1) / atomic_read(&c0) : 0,
						"c2", atomic_read(&c0) ? atomic64_read(&c2) / atomic_read(&c0) : 0,
						"c3", atomic_read(&c0) ? atomic_read(&c3) / atomic_read(&c0) : 0,
						"ns", 0);
		seq_printf(seq, "%3s %12d  %3s %-12d  %3s %-12ld  %2s %-12d  %2s %-12d\n",
						"avg", 0,
						"c1", atomic_read(&c0) ? (atomic_read(&c1) / atomic_read(&c0))/1000 : 0,
						"c2", atomic_read(&c0) ? (atomic64_read(&c2) / atomic_read(&c0))/1000 : 0,
						"c3", atomic_read(&c0) ? (atomic_read(&c3) / atomic_read(&c0))/1000 : 0,
						"us", 0);
	} else {
...
#if 0
/* For tracking page_is_mine case over time */
...
/* Dynamic threshold for controling max # of prefetch requests in the system */
inline void max_ongoing_pf_req_inc(atomic_t *max_ongoing_pf_req)
{
#ifdef CONFIG_POPCORN_PREFETCH
	atomic_inc(max_ongoing_pf_req);
    PFPRINTK("max_ongoing_pf_req %d\n", max_ongoing_pf_req->counter);
#endif
}

inline void max_ongoing_pf_req_dec(atomic_t *max_ongoing_pf_req)
{
#ifdef CONFIG_POPCORN_PREFETCH
	atomic_dec(max_ongoing_pf_req);
    PFPRINTK("max_ongoing_pf_req %d\n", max_ongoing_pf_req->counter);
#endif
}
...
static struct fault_handle *__start_invalidation(struct task_struct *tsk, unsigned long addr, spinlock_t *ptl)
{
	...
	hlist_for_each_entry(fh, &rc->faults[fk], list) {
		if (fh->addr == addr) {
			...
			/* TODO: coalse with RELEASE? or rety
					RELEASE doesn't colease with any currently */
	...
	if (found) {
		...
		PGPRINTK(" =[%d] %lx %p\n", tsk->pid, addr, fh);
		spin_lock(ptl);
	} else { /* pf: Being synchronized by origin no longer holds */
		fh = NULL;
		PGPRINTK(" =[%d] %lx\n", tsk->pid, addr);
	}
	return fh;
...
int process_madvise_release_from_remote(vma_op_request_t *req,
			int from_nid, unsigned long addr, unsigned long end)
{
	BUG();
#if 0
...
/* per vm */
struct prefetch_madvise *prefetch_deq(struct mm_struct *mm)
{
	bool found = false;
	struct prefetch_madvise *curr, *tmp;

	/* spining overhead << remote page fault overhead */
	spin_lock(&mm->remote->pf_req_lock);
	list_for_each_entry_safe(curr, tmp, &mm->remote->pf_req_list, list) {
	...
	spin_unlock(&mm->remote->pf_req_lock);
...
//	if (TRANSFER_PAGE_WITH_RDMA) { //pf_rdma
							...
							struct fault_handle *fh)
{
	if (!slot) { /* first one should create a pf ongoing mapping */
		*pf_map = kzalloc(sizeof(struct pf_ongoing_map), GFP_ATOMIC);
		BUG_ON(!(*pf_map));
		spin_lock(&fh->rc->pf_ongoing_lock);
		(*pf_map)->pf_req_id = ++g_pf_req_id;
		list_add_tail(&(*pf_map)->list, &fh->rc->pf_ongoing_list);
		spin_unlock(&fh->rc->pf_ongoing_lock);
	}
						...
						struct pf_ongoing_map **pf_map)
{
	...
	struct fault_handle *fh;
	struct remote_context *rc;
	bool found = false, leader = false, is_last = false;
	unsigned long addr =
		curr_madvise->start_addr + (curr_madvise->offset * PAGE_SIZE);
	bool is_write = curr_madvise->is_write;
	bool is_besteffort = curr_madvise->is_besteffort;
	...
	page_server_pf_action_record(is_write ? MADV_WRITE : MADV_READ);
 ...
 */
unsigned long select_prefetch_pages(struct mm_struct *mm, int *pf_nr_pages,
									struct prefetch_list *pf_list, bool async)
{
	int pf_req_id = 0;
	int try_cnt = 0;
	bool is_last = false;
...
#ifdef CONFIG_POPCORN_STAT
	ktime_t start1, end1, start2, end2, start3, end3;
#endif

	KTIME_START(start1);
	if (!async) { /* async must do */
		/* Controlling max request on the fly */
		if (atomic_read(&mm->remote->pf_ongoing_cnt) >=
			mm->remote->max_ongoing_pf_req.counter)
			goto out;
	}

	curr_madv = prefetch_deq(mm);
	if (!curr_madv) {
		goto out;
	}
	KTIME_END(start1, end1, a1);


	KTIME_START(start2);
	while (*pf_nr_pages < MAX_PF_REQ &&
			try_cnt < MAX_TRY_MADVISE_REQ) {
		if (select_prefetch_page(curr_madv, mm,
				pf_nr_pages, pf_list, &pf_map)) {
			is_last = true;
			kfree(curr_madv);
			break;
		}
	...
	KTIME_END(start2, end2, a2);
#ifdef CONFIG_POPCORN_STAT
	atomic_inc(&a0); /* overestimating for a1 & underestimating for a3, fine */
#endif
	...
	if (!is_last) { /* leftover */
		...
		list_for_each_entry_safe(this_madv, tmp, &mm->remote->pf_req_list, list) {
		...
		list_add(&curr_madv->list, &mm->remote->pf_req_list);
		spin_unlock(&mm->remote->pf_req_lock);
	}

	if (*pf_nr_pages) {
		if (!async) { /* async path has hold the cnt */
			atomic_inc(&mm->remote->pf_ongoing_cnt);
		}
		pf_req_id = pf_map->pf_req_id;
		PFPRINTK("[%d] %d ongoing pf left\n",
				current->pid, atomic_read(&mm->remote->pf_ongoing_cnt));
#ifdef CONFIG_POPCORN_CHECK_SANITY
		BUG_ON(*pf_nr_pages > MAX_PF_REQ);
#endif
	}
	KTIME_END(start3, end3, a3);
...
static int __request_remote_page(struct task_struct *tsk, int from_nid, pid_t from_pid, unsigned long addr, unsigned long fault_flags, int ws_id, struct pcn_kmsg_rdma_handle **rh, struct mm_struct *mm)
{
...
static remote_page_response_t *__fetch_page_from_origin(struct task_struct *tsk, struct vm_area_struct *vma, unsigned long addr, unsigned long fault_flags, struct page *page, struct mm_struct *mm)
{
...
#if PREFETCH_REMORE_NONBLOCKING
static void __claim_remote_page_top(struct task_struct *tsk,
			struct mm_struct *mm, struct vm_area_struct *vma,
			unsigned long addr, unsigned long fault_flags,
			int pf_num, struct remote_remote_req_map *rr_map)
{
	int peers;
	unsigned int random = prandom_u32();
	struct remote_context *rc = __get_mm_remote(mm);
	int nid, from, from_nid;
	...
	if (test_bit(my_nid, pi)) { /* page_is_mine */
		peers--;
	}
#ifdef CONFIG_POPCORN_CHECK_SANITY
	...
	for_each_set_bit(nid, pi, MAX_POPCORN_NODES) {
		...
		if (from-- == 0) {
			from_nid = nid;
			__request_remote_page(tsk, nid, pid, addr,
						fault_flags, ws->id, &rh, NULL);
		} else {
	...
	rr_map->pi[pf_num] = pi;
	rr_map->rc[pf_num] = rc;
	rr_map->pip[pf_num] = pip;
	rr_map->ws[pf_num] = ws;
	rr_map->rh[pf_num] = rh;
	return;
}

static int __claim_remote_page_bottom(int from_nid, struct page *page,
			struct vm_area_struct *vma, struct wait_station* ws,
			unsigned long addr, unsigned long fault_flags,
			unsigned long *pi, struct page *pip,
			struct remote_context *rc, struct pcn_kmsg_rdma_handle *rh)
{
	int result;
	remote_page_response_t *rp;
	rp = wait_at_station(ws);
							...
							struct mm_struct *mm, struct remote_context *rc)
{
	...
	list_for_each_entry_safe(curr, next, preprefetch_list_head, list) {
		...
		pte_t *pte;
		spinlock_t *ptl;
		struct vm_area_struct *vma = find_vma(mm, addr); //xxx
		...
		if (leader) {
			...
			if (!test_page_owner(from_nid, mm, addr)) {
				...
				/* For pf_read, this is done */
				/* For pf_write, this is a read->write req. xxx */
...
inline static int __get_remote_page(struct mm_struct *mm,
			struct page **page, struct vm_area_struct *vma,
			unsigned long addr, pte_t *pte, struct task_struct *tsk,
			struct fault_handle *fh, bool *got_remote_page)
{
#if PREFETCH_REMOTE_REMOTE_SUPPORT
#if PREFETCH_REMOTE_REMOTE_IMM
...
#if PREFETCH_REMORE_NONBLOCKING
static void __prefetch_each_at_origin_bottom(int from_nid,
					struct mm_struct *mm, struct vm_area_struct *vma,
					struct page *page, unsigned long addr, pte_t *pte,
					spinlock_t *ptl, struct fault_handle *fh,
					int succ_pf_num, unsigned long fault_flags,
					remote_prefetch_response_t *res)
{
	...
	if (fault_for_write(fault_flags)) { /* remotefault | write */
...
void __do_rr_bottom(struct remote_remote_req_map *rr_map,
					struct mm_struct *mm,
					struct vm_area_struct *vma,
					remote_prefetch_response_t *res,
					int from_nid, int pf_list_size)
{
#if PREFETCH_REMORE_NONBLOCKING
	int i;
	for (i = 0; i < MAX_PF_REQ; i++) {
		struct page *page;
		if (i >= pf_list_size) break;
		if (!rr_map->is_rr[i]) continue;

		PFPRINTK("rr_map#%d: %lx\n", i, rr_map->addr[i]);
		page = get_normal_page(vma, rr_map->addr[i], rr_map->pte[i]);
		BUG_ON(!page);

		__claim_remote_page_bottom(from_nid, page, vma,
						rr_map->ws[i], rr_map->addr[i],
						rr_map->fault_flags[i], rr_map->pi[i], 
						rr_map->pip[i], rr_map->rc[i], rr_map->rh[i]);

		__prefetch_each_at_origin_bottom(from_nid, mm, vma, page,
							rr_map->addr[i], rr_map->pte[i],
							rr_map->ptl[i], rr_map->fh[i],
							rr_map->succ_pf_num[i],
							rr_map->fault_flags[i],
							res);
	}
#endif
	return;
...
static void __prefetch_each_at_origin(struct mm_struct *mm,
					struct vm_area_struct *vma, struct task_struct *tsk,
					struct remote_context *rc, struct prefetch_body *list_curr,
...
//	}
	...
	if (found) {
		...
		if (page_is_mine(mm, addr)) { /* TODO: buffer this */
			leader = true;
			...
			*res_size += PAGE_SIZE;
			...
			if (!__prepare_get_remote_page(&fh, tsk, addr, rc, &leader,
				...
				*res_size += PAGE_SIZE;
				...
				res->result[pf_num] = PREFETCH_FAIL; /* redundant */
			}
		}
	}
	spin_unlock_irqrestore(&rc->faults_lock[fk], flags);
	...
	if (test_page_owner(from_nid, mm, addr)) { /* R->W */
		...
		if (!page_is_mine(mm, addr)) { /* remote-remote: X->R/W */
			...
			if (!__get_remote_page(mm, &page, vma, addr,
				...
				res->result[pf_num] = PREFETCH_SUCCESS;
#else
			__claim_remote_page_top(tsk, mm, vma, addr,
									fh->flags, pf_num, rr_map);
			rr_map->is_rr[pf_num] = true;
			rr_map->addr[pf_num] = addr;
			rr_map->fh[pf_num] = fh;
			rr_map->pte[pf_num] = pte;
			rr_map->ptl[pf_num] = ptl;
			rr_map->succ_pf_num[pf_num] = succ_pf_num;
			rr_map->fault_flags[pf_num] = fh->flags;
	...
	if (is_write) { /* remotefault | write */
		clear_page_owner(my_nid, mm, addr); /* I can't be owner, but from_nid */
		entry = pte_make_invalid(entry);
	} else { /* remotefault | read */
		entry = pte_make_valid(entry); /* For remote-claimed case */
		entry = pte_wrprotect(entry);
...
next:
	...
	PFPRINTK("handled pf r%d-#%d:\t%lx (%s) post(%s)\n",
			from_nid, pf_num, addr,
			res->result[pf_num] & PREFETCH_SUCCESS ? "O" : "X",
			rr_map->is_rr[pf_num] ? "O" : "X");
}
...
//	}
	...
	__do_rr_bottom(&rr_map, mm, vma, res, from_nid, pf_list_size);
	KTIME64_END(start2, end2, c2);
...
{
	struct prefetch_madvise *curr_madv = NULL, *tmp_madv;
	struct pf_ongoing_map *curr_pf_on = NULL, *tmp_pf_on;
	spin_lock(&rc->pf_req_lock);
	list_for_each_entry_safe(curr_madv, tmp_madv, &rc->pf_req_list, list) {
		list_del(&curr_madv->list);
	}
	spin_unlock(&rc->pf_req_lock);

	spin_lock(&rc->pf_ongoing_lock);
	list_for_each_entry_safe(curr_pf_on, tmp_pf_on, &rc->pf_ongoing_list, list) {
			list_del(&curr_pf_on->list);
	}
...
long page_server_prefetch_enq(unsigned long start, unsigned long end, int behavior)
{
	...
	spin_lock(&current->mm->remote->pf_req_lock);
	list_for_each_entry_safe(this, tmp, &current->mm->remote->pf_req_list, list) {
	...
	if (!is_rep) {
		/* optimization - sorting by different pid for parallel exec */
		list_add_tail(&curr->list, &current->mm->remote->pf_req_list);
	}
	spin_unlock(&current->mm->remote->pf_req_lock);
 ...
 */
void __async_prefetch_request(struct mm_struct *mm, struct task_struct *tsk)
{
	...
	if (pf_nr_pages > 0) {
		...
		atomic_dec(&mm->remote->pf_ongoing_cnt);
		pcn_kmsg_put(req);
		goto out;
	}
...
static void process_remote_prefetch_response(struct work_struct *work)
{
	...
	spin_lock(&mm->remote->pf_ongoing_lock);
	list_for_each_entry_safe(curr, tmp, &mm->remote->pf_ongoing_list, list) {
	...
	spin_unlock(&mm->remote->pf_ongoing_lock);

#ifdef CONFIG_POPCORN_CHECK_SANITY
	if (!found) { /* do_exit() has cleaned everything (not yet implemented) */
		PFPRINTK("%d %d res->[0]%lx W(%s) CONTINUE(%s) SUCC(%s)\n",
						res->origin_pid, res->remote_pid,
						first_addr, res->is_write[0] ? "O" : "X",
						res->result[0] & VM_FAULT_CONTINUE ? "O" : "X",
						res->result[0] & PREFETCH_SUCCESS ? "O" : "X");
	...
	while (pf_num < MAX_PF_REQ && pf_num < curr->pf_list_size) {
		...
		/* optimization: get all pte at once outside this loop */
		pte = __get_pte_at(mm, addr, &pmd, &ptl);
		if (!pte) {
			PGPRINTK("  [%d] No PTE!!\n", tsk->pid);
			BUG();
		...
		/* optimization: then see if I can hide this. Perf it first!! */
		if (pte_none(*pte) ||
			!(page = vm_normal_page(vma, addr, *pte))) {
			page = alloc_page_vma(GFP_HIGHUSER_MOVABLE, vma, addr);
		...
		if (result & VM_FAULT_CONTINUE) {
			...
			/* similar to __fetch_page_from_origin() */
			paddr = kmap(page);
//			if (TRANSFER_PAGE_WITH_RDMA) { //pf_rdma
//				BUG_ON(!curr->rh);
...
next:
		PFPRINTK("recv: #%d >%lx (%s) %p fh %p [%d]\n", pf_num, addr,
				result & PREFETCH_SUCCESS ? "O" : "X", curr, curr->fh[pf_num], curr->pf_list_size);
		__pf_finish(result, ret, curr, pf_num);
		pf_num++;
	}
	up_read(&mm->mmap_sem);
	kfree(curr);
	KTIME_END(start2, end2, b2);
...
pf_done:
	...
	if (tsk) put_task_struct(tsk);
#else
	__async_prefetch_request(mm, tsk);
#endif
    END_KMSG_WORK(res);
}
